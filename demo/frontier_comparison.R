...
summary(sfa(log( output ) ~ log( capital ) + log( labour ), data=front41Data, truncNorm=T))
summary(spfrontier(log( output ) ~ log( capital ) + log( labour ), data=front41Data, inefficiency="truncated"))
...
for(m in mu){
    for(s in sigma){
        z[i,j]<-logLikelihood(log( output ) ~ log( capital ) + log( labour ), data=front41Data, inefficiency="truncated",values=c(0.46553,0.28320,0.54096,0.22744,s,m))
        j <- j + 1
    }
    i <- i + 1
    j <- 1
